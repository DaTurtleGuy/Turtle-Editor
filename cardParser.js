...
function getCharacterDataFromFile(file) {
  return new Promise((resolve, reject) => {
    try {
      if (!file) {
        ...
        extractCharacterData(file)
          .then((characterData) => {
            ...
            reduceImageSize(file)
              .then((reducedImageData) => {
                // Add reduced image data to character data
                characterData.reducedImageData = reducedImageData;
                resolve(characterData);
              })
              .catch((error) => {
                reject(new Error("Error reducing image size"));
              });
          })
          .catch((error) => {
            reject(new Error("Error extracting character data from PNG"));
          });
      } else {
        reject(
          new Error(
